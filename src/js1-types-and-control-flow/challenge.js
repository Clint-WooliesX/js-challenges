...
export const findType = () => {
  var typeIs = typeof thing;
  if (typeIs === "string" || typIs === "number" || typeIs === "boolean") {
    return "This is a " + typeIs;
  }
...
export const getIsValidOnNameTag = () => {
  if (nameTagOption.length <= 8 && nameTagOption[0].toUpperCase() === nameTagOption[0]) {
    return true;
  }
    return false;
};
...
export const getHasUppercaseLetters = () => {
  if (/^[A-Z]/.test(stringWithUppercaseLetters)) {
    return true;
  }
};
...
export const convertPascalCaseToSnakeCase = () => {
  ...
  const joinedWords = splitWords.join("_");
  const joinedWords2snakeCase = joinedWords.toLowerCase();
  return joinedWords2snakeCase;
};
